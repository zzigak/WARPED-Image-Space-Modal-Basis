...
function simulation() {
    ...
    MS.simulate13(modes, timePassed);
    drawSimulationState();
...
function applyForces(currentMousePosition = [0, 0]) {
    ...
    MS.applyInstantaneousForce(selectedModes, d, p, modes);
...
function initializeModes(modeBasis, modeFrequencies, topKValuesWeights, ampPhaseRanges, ranges, urls) {
    ...
    for (let i = 0; i < Math.min(k,modeBasis.length); i++) {
        ...
        let strength = topKValuesWeights[i] * 100;
