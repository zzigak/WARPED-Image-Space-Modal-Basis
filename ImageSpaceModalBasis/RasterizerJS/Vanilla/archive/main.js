...
// State Update Stuff
import * as MS from './stateUpdate_new.js';
...
function simulation() {
    ...
    if (selectedModesList.length  > 0) {
        ...
        MS.simulate13(selectedModesList, currentTime, isMouseDown, speed);
...
const accumulationVert = loadShader(gl, gl.VERTEX_SHADER, accumulationVertSource);
    ...
    const modeData = await fetchData('./assets/wireman/wireman_modes_aug30/modes_data_aug30_wireman.json');
...
function handleMouseUp() {
    ...
    MS.applyInstantaneousForce(selectedModesList, [0,0],[0,0], window.performance.now());
...
function applyForces(currentMousePosition) {
    ...
    MS.applyInstantaneousForce(selectedModesList, dNorm, point, window.performance.now()); 
}
