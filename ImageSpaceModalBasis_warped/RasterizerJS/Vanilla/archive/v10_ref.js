...
function initializeModes(modeBasis, modeFrequencies, topKValuesWeights, ranges, ampPhaseRanges, urls) {
    ...
    for (let i = 0; i < modeBasis.length; i++) {
        // max, min real
        // let minReal = ranges[0];
        // let maxReal = ranges[1];
        // let minImag = ranges[2];
        // let maxImag = ranges[3];

        // 0th and 2nd channel with real ranges, 1st and 3rd channel with imaginary ranges
        // let basis = modeBasis[i].map((pixel) => {
        //     return [
        //         pixel[0] * (maxReal - minReal) + minReal,
        //         pixel[1] * (maxImag - minImag) + minImag,
        //         pixel[2] * (maxReal - minReal) + minReal,
        //         pixel[3] * (maxImag - minImag) + minImag

        //     ];
        // });

        let modeI = modeBasis[i];

        const minAmp = ampPhaseRanges[0];
        const maxAmp = ampPhaseRanges[1];
        const minPhase = ampPhaseRanges[2];
        const maxPhase = ampPhaseRanges[3];
        // console.log("minAmp, maxAmp, minPhase, maxPhase", minAmp, maxAmp, minPhase, maxPhase);

        const ampRange = maxAmp - minAmp;
        const phaseRange = maxPhase - minPhase;

        // Create a new array with the same structure as modeBasis
        let denormalizedBasis = modeI.map(row =>
            row.map(pixel => [
                pixel[0] * ampRange + minAmp,   // 0th channel (amplitude)
                pixel[1] * phaseRange + minPhase, // 1st channel (phase)
                pixel[2] * ampRange + minAmp,   // 2nd channel (amplitude)
                pixel[3] * phaseRange + minPhase  // 3rd channel (phase)
            ])
        );

        // console.log("denormalized basis max and min,", Math.max(...denormalizedBasis.flat(1)), Math.min(...denormalizedBasis.flat(1)));
        console.log(getArrayShape(denormalizedBasis));
        ...
        let mass = 1;
        let url = urls[i];
        modes.push(new MS.Mode(denormalizedBasis, strength, frequency, damping, mass, url));
    }
...
async function init() {
    ...
    const deformationVertSource = await loadShaderFile('deformation-vertex.vert');
    const deformationFragSource = await loadShaderFile('deformation-fragment.frag');
    ...
    const modeData = await fetchData('./assets/modes_data_normalized.json');
    console.log("Got mode data");
    initializeModes(modeData.modeBasis, modeData.modeFrequencies, modeData.topKValuesWeights, modeData.ranges, modeData.ampPhaseRanges, modalUrls);
